  ...
  <div>
    ...
    >
      <div
        ref="desc"
        @click="open"
        v-html="$t('c2p_save_card_desc', { click2pay })"
      />
    </f-form-group>
...
export default {
  ...
  methods: {
    open(ev) {
      if (ev.target !== this.$refs.desc.querySelector('a')) return
      ev.preventDefault()
    ...
    input(value) {
      if (value) {
        this.$set(this.params, 'click2pay_checkout_data', {
          consumer: {
          ...
          complianceSettings: {
            complianceResources: this.complianceResources,
          },
        })
      } else {
    ...
    watchRememberMe() {
      this.params.click2pay_checkout_data.complianceSettings.complianceResources =
        this.complianceResources
    },
  },
}
</script>

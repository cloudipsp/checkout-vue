<template>
  <div class="f-info-wrapper">
    ...
    <div v-if="options.fee" class="f-block f-block-hr">
      <div><span v-t="'amount'"></span> {{ amount }} <span v-t="form.currency"></span></div>
      <div v-if="form.fee">
        <span v-t="'fee'"></span> <span v-if="amount_fee">{{amount_fee}} <span v-t="form.currency"></span></span> <span v-if="fee">({{ fee }})</span>
      </div>
    </div>
